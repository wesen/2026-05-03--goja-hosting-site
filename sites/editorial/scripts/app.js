...
function validStatus(status) {
  return columns.some(([value]) => value === status) ? status : "pitch";
}
...
function migrate() {
  ...
  ignoreDuplicateColumn(() =>
    db.exec(
      "ALTER TABLE editorial_cards ADD COLUMN format TEXT NOT NULL DEFAULT 'Feature'",
    ),
  );
  ignoreDuplicateColumn(() =>
    db.exec(
      "ALTER TABLE editorial_cards ADD COLUMN channel TEXT NOT NULL DEFAULT 'Web'",
    ),
  );
  ignoreDuplicateColumn(() =>
    db.exec(
      "ALTER TABLE editorial_cards ADD COLUMN priority TEXT NOT NULL DEFAULT 'Normal'",
    ),
  );
  ignoreDuplicateColumn(() =>
    db.exec(
      "ALTER TABLE editorial_cards ADD COLUMN deadline TEXT NOT NULL DEFAULT ''",
    ),
  );
  ignoreDuplicateColumn(() =>
    db.exec(
      "ALTER TABLE editorial_cards ADD COLUMN checklist_done INTEGER NOT NULL DEFAULT 0",
    ),
  );
  ignoreDuplicateColumn(() =>
    db.exec(
      "ALTER TABLE editorial_cards ADD COLUMN checklist_total INTEGER NOT NULL DEFAULT 4",
    ),
  );
}
...
function seedIfEmpty(session) {
  ...
  const rows = db.query(
    "SELECT COUNT(*) AS count FROM editorial_cards WHERE session_id = ?",
    sid,
  );
  if (rows.length && Number(rows[0].count || 0) === 0) {
    const cards = [
      [
        "Interview: Goja as a CMS",
        "A Q&A about letting trusted JavaScript own product workflows without building a bespoke SPA.",
        "pitch",
        10,
        "Interview",
        "Mira",
      ...
      [
        "Kanban DSL launch essay",
        "Narrative launch article with diagrams, callback timeline, and production screenshots.",
        "outline",
        10,
        "Architecture",
        "Manuel",
      ...
      [
        "SQLite guard field report",
        "Explain soft cleanup callbacks, hard-limit rejection, and the operational design trade-offs.",
        "draft",
        10,
        "Backend",
        "Ada",
      ...
      [
        "K3s deployment incident note",
        "Short runbook update about local-path WaitForFirstConsumer and Argo sync waves.",
        "copyedit",
        10,
        "Ops",
        "Lin",
      ...
      [
        "Field Notes board walkthrough",
        "Screenshot-led post showing drag/drop, server callbacks, and session-isolated data.",
        "published",
        10,
        "Demo",
        "Casey",
    ...
    cards.forEach((c) =>
      db.exec(
        `INSERT INTO editorial_cards(session_id, title, description, status, position, tag, owner, format, channel, priority, deadline, checklist_done, checklist_total, done)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        sid,
        ...c,
      ),
...
function listCards(session, filters) {
  ...
  if (filters.search) {
    ...
    where.push(
      "(lower(title) LIKE ? OR lower(description) LIKE ? OR lower(tag) LIKE ? OR lower(owner) LIKE ? OR lower(format) LIKE ? OR lower(channel) LIKE ?)",
    );
  ...
  return db.query(
    `SELECT * FROM editorial_cards WHERE ${where.join(" AND ")} ORDER BY position, id`,
    ...args,
  );
}
function listColumn(session, status) {
  return db.query(
    "SELECT * FROM editorial_cards WHERE session_id = ? AND status = ? ORDER BY position, id",
    sessionId(session),
    status,
  );
}
function normalizeColumn(session, status) {
  ...
  listColumn(sid, status).forEach((card, index) =>
    db.exec(
      "UPDATE editorial_cards SET position = ? WHERE session_id = ? AND id = ?",
      (index + 1) * 10,
      sid,
      card.id,
    ),
...
function moveCard({ session, id, toStatus, toIndex }) {
  const sid = sessionId(session);
  id = Number(id);
  toStatus = validStatus(String(toStatus || "pitch"));
  toIndex = Number.isFinite(Number(toIndex)) ? Number(toIndex) : 0;
  const existing = db.query(
    "SELECT * FROM editorial_cards WHERE session_id = ? AND id = ?",
    sid,
    id,
  )[0];
  ...
  const destination = db.query(
    "SELECT * FROM editorial_cards WHERE session_id = ? AND status = ? AND id != ? ORDER BY position, id",
    sid,
    toStatus,
    id,
  );
  ...
  db.exec(
    "UPDATE editorial_cards SET status = ?, done = ?, updated_at = CURRENT_TIMESTAMP WHERE session_id = ? AND id = ?",
    toStatus,
    done,
    sid,
    id,
...
function nextPosition(session, status) {
  const rows = db.query(
    "SELECT COALESCE(MAX(position), 0) AS max_position FROM editorial_cards WHERE session_id = ? AND status = ?",
    sessionId(session),
    status,
  );
...
function searchText(card) {
  return `${card.title || ""} ${card.description || ""} ${card.tag || ""} ${card.owner || ""} ${card.format || ""} ${card.channel || ""}`.toLowerCase();
}
function progress(card) {
  const total = Math.max(1, Number(card.checklist_total || 1));
  return Math.round((Number(card.checklist_done || 0) / total) * 100);
}
function summary(session) {
  ...
  return {
    ...
    urgent: rows.filter((c) => String(c.priority).toLowerCase() === "urgent")
      .length,
    avgProgress: rows.length
      ? Math.round(rows.reduce((a, c) => a + progress(c), 0) / rows.length)
      : 0,
  };
}
    ...
    render
      .toolbar((ctx) =>
        ui.form(
          ...
          ui.input({
            id: "search",
            name: "search",
            placeholder: "Search by owner, channel, format...",
      ...
      .card((card) =>
        ui.fragment(
          ui.div(
            { class: "story-top" },
            ui.span(card.format || "Feature"),
            ui.span(card.channel || "Web"),
          ),
          ui.div(
            { class: "story-body" },
            ui.h3(card.title),
            ...
            ui.div(
              { class: "progress", title: progress(card) + "% complete" },
              ui.div({ class: "bar", style: `width:${progress(card)}%` }),
            ),
            ui.div(
              { class: "card-meta" },
              ui.span({ class: "pill" }, "✍ " + (card.owner || "Unassigned")),
              ui.span(
                {
                  class: "pill " + (card.priority === "Urgent" ? "urgent" : ""),
                },
                card.priority || "Normal",
              ),
              ui.span({ class: "pill" }, card.tag || "Essay"),
              ui.span({ class: "pill" }, card.deadline || "No date"),
            ),
          ),
        ),
      )
      .emptyColumn((column) =>
        ui.div({ class: "empty" }, "No copy on this desk."),
      ),
  )
  .actions((actions) =>
  ...
  return ui.page(
    ...
    ui.main(
      ...
      ui.header(
        { class: "hero" },
        ui.div(
          ui.h1({ class: "masthead" }, "Editorial Desk"),
          ui.p(
            { class: "subtitle" },
            "A newsroom-style board with deadlines, owners, channels, formats, and checklist progress.",
          ),
        ),
        ui.div(
          { class: "metrics" },
          ui.div({ class: "metric" }, ui.strong(s.total), ui.span("stories")),
          ui.div(
            { class: "metric" },
            ui.strong(s.published),
            ui.span("published"),
          ),
          ui.div({ class: "metric" }, ui.strong(s.urgent), ui.span("urgent")),
          ui.div(
            { class: "metric" },
            ui.strong(s.avgProgress + "%"),
            ui.span("avg done"),
          ),
        ),
      ),
      ui.form(
        ...
        ui.input({ name: "description", placeholder: "Nut graf" }),
        ui.select(
          { name: "status" },
          columns.map(([value, label]) => ui.option({ value }, label)),
        ),
        ui.input({ name: "owner", placeholder: "Owner" }),
        ui.input({ name: "format", placeholder: "Format", value: "Feature" }),
        ui.input({ name: "deadline", placeholder: "Deadline" }),
        ui.button({ type: "submit" }, "File"),
      ),
      board.render({
...
app.post("/cards", (req, res) => {
  ...
  const status = validStatus(String(body.status || "pitch"));
  db.exec(
    "INSERT INTO editorial_cards(session_id, title, description, status, position, tag, owner, format, channel, priority, deadline, checklist_done, checklist_total, done) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
    sessionId(req.session),
    title,
    String(body.description || ""),
    status,
    nextPosition(req.session, status),
    String(body.tag || "Essay"),
    String(body.owner || ""),
    String(body.format || "Feature"),
